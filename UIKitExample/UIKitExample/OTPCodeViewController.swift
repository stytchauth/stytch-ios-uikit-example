...
final class OTPCodeViewController: UIViewController {
    ...
    private let errorLabel: UILabel = {
        ...
        label.text = "Invalid passcode, please try again."
        label.isHidden = true
        return label
    }()
    ...
    func configure(
        ...
        let attributedText = NSMutableAttributedString("A 6-digit passcode was sent to you at ")
    ...
    @objc private func updateExiryText() {
        guard
            ...
            expiryButton.setAttributedTitle(
                expiryAttributedText(initialSegment: "Your code has expired."),
                for: .normal
            )
            timer?.invalidate()
            return
        ...
        expiryButton.setAttributedTitle(
            expiryAttributedText(initialSegment: "Your code expires in \(dateString)."),
            for: .normal
        )
    }
    ...
    private func expiryAttributedText(initialSegment: String) -> NSAttributedString {
        let attributedString = NSMutableAttributedString(string: initialSegment + " Didn't get it?", attributes: [.font: UIFont.systemFont(ofSize: 16)])
        let appendedAttributedString = NSAttributedString(string: " Resend it.", attributes: [.font: UIFont.systemFont(ofSize: 16, weight: .semibold)])
